  ...
  return (
    <article className="industrial-card flex h-full flex-col overflow-hidden transition duration-300 hover:-translate-y-1 hover:shadow-panel">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-2 text-sm font-medium text-primary">{product.company}</p>
        <p className="mt-3 flex-1 text-sm leading-7 text-ink-muted">{product.description}</p>
        <div className="mt-6 flex items-center justify-between border-t border-line pt-4">
          <span className="text-xs font-semibold uppercase tracking-[0.22em] text-ink-muted">{product.model}</span>
          <button className="inline-flex items-center gap-1 text-sm font-bold text-primary transition hover:gap-2" type="button">
            查看规格
